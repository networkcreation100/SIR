...
function isEmail(value) { return !getEmailValidationError(value); }
...
function normalizePhoneRecipientValue(value = '') {
  const text = String(value || '').trim();
  const digits = text.replace(/\D/g, '');
...
function formatRecipientLabel(contact) {
  return contact.name ? `${contact.name} — ${contact.value}` : contact.value;
}

function formatRecipientInput(contact) {
  return contact.name ? `${contact.name} <${contact.value}>` : contact.value;
}

function classifyRecipients(input = '') {
...
function smartFormatRecipients(input) {
...
function rowsFromRecipientText(text = '') {
...
function classifyRecipientRows(rows = []) {
  const invalid = [];
  const contacts = [];
  const duplicates = [];
  const seen = new Set();
