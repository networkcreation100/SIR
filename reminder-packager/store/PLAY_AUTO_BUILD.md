...
Then base64-encode it so it can live safely in a GitHub secret:
    ...
    # macOS / Linux
    base64 -i sir-upload.jks | tr -d '\n' > sir-upload.jks.base64.txt

    # Windows PowerShell
    [Convert]::ToBase64String([IO.File]::ReadAllBytes("sir-upload.jks")) > sir-upload.jks.base64.txt
...
| Secret name                 | Value                                             |
|-----------------------------|---------------------------------------------------|
| ANDROID_KEYSTORE_BASE64     | contents of `sir-upload.jks.base64.txt`           |
